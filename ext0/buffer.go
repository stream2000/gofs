...
func (b *ExtendedBuffer) Init(size int, ino *Ext0Inode) {
...
func (b *ExtendedBuffer)allocNewBlock(num int){
	for i := 0; i < num; i++ {
...
// 向后写
func (b *ExtendedBuffer) Write(data []byte) int {
	if len(data) == 0 {
		// nothing to write
		return 0
	}
...
func (b *ExtendedBuffer) modify(offset int, data []byte) {
...
func (b *ExtendedBuffer) setDataInsideBlock(num int, offsetInBlock int, data []byte) int {
